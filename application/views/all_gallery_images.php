...
<head>
  ...
  <title>Embroidery | Gallery images</title>
        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
              ...
              <h3 class="box-title">Gallery Images of <span style="color:green"><?php echo ($product_name->product_name);?> </span ></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
			<?php //var_dump($all_staff->result_array());?>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
				<?php
					//var_dump($gallery->result_array());
						if(isset($gallery) && $gallery!=false){
						$count=0;
						foreach($gallery->result_array() as $row){ ?>
                <tr>
                  <td><?php echo ++$count;?></td>
                  <td><img src="<?php echo base_url($row['p_g_name'])?>"  width="100px" align="center"/></td>
                 
						<?php 
									$u_id=$this->encrypt->encode($row['p_g_id']);
									 $u_id = strtr(
									$u_id,
									array(
										'+' => '.',
                 ...
                 <td>
					<a onClick="edit_product(<?php echo $row['p_g_id']; ?>)" style="cursor:pointer;color:green" href="#popup3" data-toggle="modal"> Edit image</a>
                  ...
                  <td>
						...
						?>
								<a title="delete image" onclick="return confirm('Are you sure?')" href="<?php echo site_url('Product/delete_gallery_image')?>/<?php echo $u_id;?>"><span class="glyphicon glyphicon-trash"></span></td>
						<?php } ?>
			  </tr>
  		...
  		document.getElementById("detail_edit").innerHTML = "<center>Wait Loading..</center>";
			var user_id=d;
			 ...
			 // Create our XMLHttpRequest object
					...
					var url ="<?php echo site_url('Product/edit_gallery_image_display')?>/"+user_id;
				//	alert(url);
  		...
  		document.getElementById("detail").innerHTML = "<center>Wait Loading..</center>";
			var user_id=d;
			 ...
			 // Create our XMLHttpRequest object
					var hr = new XMLHttpRequest();
					// Create some variables we need to send to our PHP file
					var url ="<?php echo site_url('Product/full_product_detail')?>/"+user_id;
				//	alert(url);
					hr.open("POST", url, true);
					// Set content type header information for sending url encoded variables in the request
					hr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
