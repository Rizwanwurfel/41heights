...
<head>
  ...
  <title>Uniscreen | food attribute values</title>
...
<div class="wrapper">
 ...
 <?php 
		...
		$this->session->set_userdata('page','all_food_attr_v');
	?>
	...
	<!----------------sidebar-------------------------->
		...
		$this->session->unset_userdata('page','all_food_attr_v');
	?>
			...
			<?php //var_dump($all_staff->result_array());?>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?php echo $row['f_att_v_name'];?> </td>
                  <td><a onClick="full_items_det(<?php echo $row['f_att_v_id']; ?>)" style="cursor:pointer" href="#full_items" data-toggle="modal"> view all values</a> </td>
                  
                  <td><a onClick="edit_att(<?php echo $row['f_att_v_id']; ?>)" style="cursor:pointer" href="#popup3" data-toggle="modal"> Edit</a></td>
                  <td>
						<?php if( $this->session->userdata('user_role_uns')=="admin" ) { echo "-"; } else{ 
						
							$u_id=$this->encrypt->encode($row['f_att_v_id']);
						 $u_id = strtr(
						$u_id,
						array(
							'+' => '.',
  		...
  		document.getElementById("detail_edit").innerHTML = "<center>Wait Loading..</center>";
			var user_id=d;
			 ...
			 // Create our XMLHttpRequest object
					var hr = new XMLHttpRequest();
					// Create some variables we need to send to our PHP file
					var url ="<?php echo site_url('Menu/full_detail_att_values')?>/"+user_id;
				//	alert(url);
					hr.open("POST", url, true);
					// Set content type header information for sending url encoded variables in the request
					hr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
