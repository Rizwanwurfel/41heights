...
<head>
  ...
  <title>Embroidery | All sub cat</title>
...
<div class="wrapper">
 ...
 <?php 
		...
		$this->session->set_userdata('main_sub','sub_cats');
		$this->session->set_userdata('page','all_sub_cat');
	?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">All sub cat</li>
      </ol>
    </section>
		<?php 
			...
			<?php //var_dump($all_staff->result_array());?>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th>sub cat image</th>
                  <th>sub cat name</th>
                  <th>cat parent name</th>
                  <th>sub cat description</th>
                  <th>Edit</th>
                  <th>Delete</th>
                </tr>
                </thead>
				...
				<?php
						...
						$count=0;
						foreach($all_sub_cat->result_array() as $row){ ?>
                <tr>
                  <td><?php echo ++$count;?></td>
                  <td><img src="<?php echo base_url($row['sub_cat_image'])?>"  width="60px" align="center"/></td>
                  <td><?php echo $row['sub_cat_name'];?> </td>
                  <td><?php echo $row['m_cat_name'];?> </td>
                  <td><?php echo $row['sub_cat_desc'];?></td>
                  <td><a onClick="edit_cat(<?php echo $row['sub_cat_id']; ?>)" style="cursor:pointer" href="#popup3" data-toggle="modal"> Edit</a></td>
                  <td>
						<?php if( $this->session->userdata('user_role_emb')=="admin" ) { echo "-"; } else{ 
						
							$u_id=$this->encrypt->encode($row['sub_cat_id']);
						 $u_id = strtr(
						$u_id,
						array(
							'+' => '.',
						...
						?>
								<a title="delete record" onclick="return confirm('Are you sure?')" href="<?php echo site_url('Product/delete_sub_cat')?>/<?php echo $u_id;?>"><span class="glyphicon glyphicon-trash"></span></td>
						<?php } ?>
			  </tr>
  		...
  		document.getElementById("detail_edit").innerHTML = "<center>Wait Loading..</center>";
			var user_id=d;
			 ...
			 // Create our XMLHttpRequest object
					...
					var url ="<?php echo site_url('Product/edit_sub_cat_display')?>/"+user_id;
				//	alert(url);
