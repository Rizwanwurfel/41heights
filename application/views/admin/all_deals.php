...
<head>
  ...
  <title>41 Heights | All Deals</title>
...
<div class="wrapper">
 ...
 <?php 
		...
		$this->session->set_userdata('page','all_deals');
	?>
			...
			<?php //var_dump($all_staff->result_array());?>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><img src="<?php echo base_url($row['deal_image'])?>"  width="60px" align="center"/></td>
                  <td><?php echo $row['deal_name'];?> </td>
                  <td><?php echo $row['cat_name'];?> </td>
                  <td><?php echo "RS. ".$row['deal_price'];?> </td>
                 
						<?php 
									$u_id=$this->encrypt->encode($row['deal_id']);
									 $u_id = strtr(
									$u_id,
									array(
										'+' => '.',
						...
						?>
                  <td><a class="btn btn-small" onClick="full_detail(<?php echo $row['deal_id']; ?>)" href="#detail_pro" data-toggle="modal">View</a></td>
                  ...
                  <td>
						...
						?>
								<a title="delete record" onclick="return confirm('Are you sure?')" href="<?php echo site_url('Deals/delete_deal')?>/<?php echo $u_id;?>"><span class="glyphicon glyphicon-trash"></span></td>
						<?php } ?>
			  </tr>
  		...
  		document.getElementById("detail_edit").innerHTML = "<center>Wait Loading..</center>";
			var user_id=d;
			 ...
			 // Create our XMLHttpRequest object
					...
					var url ="<?php echo site_url('Deals/edit_deal_display')?>/"+user_id;
				//	alert(url);
  		...
  		document.getElementById("detail").innerHTML = "<center>Wait Loading..</center>";
			var user_id=d;
			 ...
			 // Create our XMLHttpRequest object
					var hr = new XMLHttpRequest();
					// Create some variables we need to send to our PHP file
					var url ="<?php echo site_url('Deals/full_deal_detail')?>/"+user_id;
				//	alert(url);
					hr.open("POST", url, true);
					// Set content type header information for sending url encoded variables in the request
					hr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
